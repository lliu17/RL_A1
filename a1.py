...
def sample_average(arm_idx, prev_reward, curr_reward, debug=False):
    ...
    estimated_rewards[arm_idx] = prev_reward + (curr_reward - prev_reward)/times_chosen[arm_idx]

def weighted_average(arm_idx, prev_reward, curr_reward, alpha, debug=False):
    estimated_rewards[arm_idx] = prev_reward + alpha * (curr_reward - prev_reward)
...
def epsilon_greedy(step_idx, exp_idx, e, method, debug=False):
    ...
    if method == "sample_average":
        # print("sample, got reward = ", curr_reward)
        sample_average(arm_idx, prev_reward, curr_reward, debug)
        # print("exp_idx=", exp_idx)
        avg_obtained_rewards_sample[step_idx] = (curr_reward + exp_idx * avg_obtained_rewards_sample[step_idx])/(exp_idx + 1)
    elif method == "weighted_average":
        # print("weighted, got reward = ", curr_reward)
        weighted_average(arm_idx, prev_reward, curr_reward, alpha, debug)
        avg_obtained_rewards_weight[step_idx] = (curr_reward + exp_idx * avg_obtained_rewards_weight[step_idx])/(exp_idx + 1)
